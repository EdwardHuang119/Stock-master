...
def ax_RSI(ax2,days):
    ...
    fillcolor = '#00ffe8'
    ax2.plot(df.date.values, df.RSI1.values, color=rsiCol, lw=2,label="$RSI1$")
    ax2.plot(df.date.values, df.RSI2.values, color=posCol, lw=2,label="$RSI2$")
    ax2.plot(df.date.values, df.RSI3.values, color=negCol, lw=2,label="$RSI3$")
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
...
def ax_BOLL(ax1,days):
    ...
    fillcolor = '#00ffe8'
    ax1.plot(df.date.values, df.BOLL_u.values, color=rsiCol, lw=2,label="$U$")
    ax1.plot(df.date.values, df.BOLL_m.values, color=posCol, lw=2,label="$M$")
    ax1.plot(df.date.values, df.BOLL_d.values, color=rsiCol, lw=2,label="$D$")
    ax1.plot(df.date.values, df.close.values, color='yellow', lw=3,label="$CLOSE$")
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
...
def ax_MACD2(ax1,days):
    ...
    fillcolor = '#00ffe8'
    ax1.plot(df.date.values, df.MACDsign_12_26.values, color=rsiCol, lw=1,label="$DIF$")
    ax1.plot(df.date.values, df.MACD_12_26.values, color=negCol, lw=1,label="$MACD$")
    ax1.fill_between(df.date.values, df.MACDdiff_12_26.values, 0, alpha=0.5, facecolor=fillcolor, edgecolor=fillcolor)
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax1.spines['bottom'].set_color("#5998ff")
    ax1.spines['top'].set_color("#5998ff")
    ax1.spines['left'].set_color("#5998ff")
...
def ax_OBV(ax2,days):
    ...
    fillcolor = '#00ffe8'
    ax2.plot(df.date.values, df.OBV_6.values, color=rsiCol, lw=2,label="$OBV6$")
    ax2.plot(df.date.values, df.OBV_12.values, color=posCol, lw=2,label="$OBV12$")
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax2.spines['bottom'].set_color("#5998ff")
    ax2.spines['top'].set_color("#5998ff")
    ax2.spines['left'].set_color("#5998ff")
...
def draw_BOLL(ax1,days,x):
    ...
    ax2 = plt.subplot2grid((7,4), (5,0), sharex=ax1, rowspan=2, colspan=4,fc=g.ubg)
    fillcolor = '#00ffe8'
    ax2.plot(df.date.values, df.BOLL_u.values, color=rsiCol, lw=2,label="$U$")
    ax2.plot(df.date.values, df.BOLL_m.values, color=posCol, lw=2,label="$M$")
    ax2.plot(df.date.values, df.BOLL_d.values, color=rsiCol, lw=2,label="$D$")
    ax2.plot(df.date.values, df.close.values, color='yellow', lw=3,label="$CLOSE$")
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax2.spines['bottom'].set_color("#5998ff")
    ax2.spines['top'].set_color("#5998ff")
    ax2.spines['left'].set_color("#5998ff")
...
def draw_KDJ(ax1,days,x,y,z):
    ...
    ax2 = plt.subplot2grid((7,4), (5,0), sharex=ax1, rowspan=2, colspan=4,fc=g.ubg)
    fillcolor = '#00ffe8'
    ax2.plot(df.date.values, df.K.values, color=rsiCol, lw=2,label="$K$")
    ax2.plot(df.date.values, df.D.values, color=posCol, lw=2,label="$D$")
    ax2.plot(df.date.values, df.J.values, color=negCol, lw=2,label="$J$")
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax2.spines['bottom'].set_color("#5998ff")
    ax2.spines['top'].set_color("#5998ff")
    ax2.spines['left'].set_color("#5998ff")
...
def draw_MACD(ax1,days,x,y,z):
    ...
    ax2 = plt.subplot2grid((7,4), (5,0), sharex=ax1, rowspan=2, colspan=4,fc=g.ubg)
    fillcolor = '#00ffe8'
    ax2.plot(df.date.values, df.MACDsign_12_26.values, color=rsiCol, lw=1,label="$DIF$")
    ax2.plot(df.date.values, df.MACD_12_26.values, color=negCol, lw=1,label="$MACD$")
