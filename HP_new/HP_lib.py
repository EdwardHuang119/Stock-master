...
def EGT(df,tp1,tp2,al=''):  
    ...
    ZB_l = []
    y=0
    while math.isnan(df[tp1][i]) or  math.isnan(df[tp2][i]) :
        y=0
        ZB_l.append(y)  
        i = i + 1   
       
    while i < len(df):  
        if (df[tp1][i]) >= df[tp2][i]:
                y=1
        else:
                y=0
        ZB_l.append(y)  
...
def CROSS(df,tp1,tp2,al=''):  
    ...
    while i < len(df):  
        ...
        if ((df[tp1].at[i-1]<df[tp2].at[i-1]) and (df[tp1].at[i] >=df[tp2].at[i])):
                y=1
        else:
                y=0
        CR_l.append(y)  
...
def OBVX(df, n,m):  
    ...
    OBV_ma = pd.Series(pd.Series.rolling(OBV, n).mean(), name = 'OBV_' + str(n))  
    df = df.join(OBV_ma)  
    #OBV_ma = pd.Series(pd.rolling_mean(OBV, m), name = 'OBV_' + str(m))  
    OBV_ma = pd.Series(pd.Series.rolling(OBV, m).mean(), name = 'OBV_' + str(m))
    df = df.join(OBV_ma)  
    return df
...
def MACD(df, n_fast, n_slow):  
    ...
    EMAfast=pd.Series.ewm(df['close'], span=n_fast, min_periods= n_slow - 1, adjust=True).mean()
    #EMAslow = pd.Series(pd.ewma(df['close'], span = n_slow, min_periods = n_slow - 1))  
    EMAslow=pd.Series.ewm(df['close'], span=n_slow, min_periods=n_slow - 1, adjust=True).mean()
    MACD = pd.Series(EMAfast - EMAslow, name = 'MACD_' + str(n_fast) + '_' + str(n_slow))  
    #MACDsign = pd.Series(pd.ewma(MACD, span = 9, min_periods = 8), name = 'MACDsign_' + str(n_fast) + '_' + str(n_slow))  
    MACDsign = pd.Series(pd.Series.ewm(MACD, span=9, min_periods=8, adjust=True).mean(), name = 'MACDsign_' + str(n_fast) + '_' + str(n_slow))  
