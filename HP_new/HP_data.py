...
def qatots(df1):
    df1a=df1.reset_index(level=None, drop=True ,col_level=0, col_fill='') 
    df1a['volume']=df1a.vol
    return df1a
...
def get_k_data(ss='000776',ktype='D',start='1991-01-01',end='2018-10-15',index=False,autype='qfq'):
    pp=''
    if autype=='None':
        pp=pp+'none\\'
    if autype=='hfq':
...
if __name__ == '__main__':
    df=get_k_data('000776')
    print(df)
    df['交易日期']=df['date']
    df=df.set_index(df['交易日期'])
    print(df[df.index=='2018-08-27'])
    print(df['2018-01-04':'2018-03-04'])
